...
static int32_t osSemaphoreWait(semaphore_handle_t semaphore_id, uint32_t timeout_ms)
{
	if (xSemaphoreTake(semaphore_id, MILLISEC_TO_TICK(timeout_ms)) != pdTRUE) {	
		return osErrorOS;										
	}														
  	else return osOK;
}

static int32_t osSemaphoreRelease(semaphore_handle_t semaphore_id)
{
  	int32_t result = osOK;
...
int control_path_platform_init(void)
{
	...
	// 	return ESP_FAIL;
	// }
	if (xSemaphoreTake( readSemaphore, ( TickType_t ) 100 ) != pdTRUE){
		printf("could not obtain readSemaphore\n\r");
		return ESP_FAIL;
	}
...
void *hosted_thread_create(void (*start_routine)(void const *), void *arg)
{
	...
	// 	esp_mem_free(thread_handle);
	// 	return NULL;
	// }

	BaseType_t xReturned = xTaskCreate(start_routine, 
							"Ctrl_port_tsk", 
							GET_TASK_STACK_LEN(CTRL_PATH_TASK_STACK_SIZE), 
							arg, 
							CTRL_PATH_TASK_PRIO, 
							thread_handle);
    if (xReturned != pdPASS){
...
unsigned int esp_sleep(unsigned int seconds) {
   //osDelay(seconds * 1000);
   vTaskDelay(pdMS_TO_TICKS(seconds * 1000));
   return 0;
}

unsigned int esp_msleep(unsigned int mseconds) {
   //osDelay(mseconds);
   vTaskDelay(pdMS_TO_TICKS(mseconds)); // because os tick freq is 100, the esp_msleep at least delay 10ms
   return 0;
}
...
int hosted_get_semaphore(void * semaphore_handle, int timeout)
{
	...
	if (!timeout) {
		...
		//return xSemaphoreTake(*sem_id, 0);
	} else if (timeout<0) {
		/* Blocking */
		return osSemaphoreWait(*sem_id, portMAX_DELAY);
		//return xSemaphoreTake(*sem_id, portMAX_DELAY);
	} else {
		//printf("hosted_get_semaphore timeout:%d\r\n", timeout);
		return osSemaphoreWait(*sem_id, SEC_TO_MILLISEC(timeout));
		//return xSemaphoreTake(*sem_id, MILLISEC_TO_TICK(SEC_TO_MILLISEC(timeout)));
	}
}
...
int hosted_timer_stop(void *timer_handle)
{
	...
	if (timer_handle) {
		...
		if (xTimerStop(((struct timer_handle_t *)timer_handle)->timer_id, 0) != pdPASS) {
      		printf("Failed to stop timer\n");
    	}
		...
		if ((xTimerDelete(((struct timer_handle_t *)timer_handle)->timer_id, portMAX_DELAY )) != pdPASS) {
      		printf("Failed to delete timer\n");
    	}

		esp_mem_free(timer_handle);
		return ret;
	}
	return ESP_FAIL;
}
		...
		void (*timeout_handler)(void const *), void *arg)
{
	...
	timer_handle->timer_id = 
			xTimerCreate((const char *)"",
				MILLISEC_TO_TICK(SEC_TO_MILLISEC(duration)), //1, // period should be filled when starting the Timer using osTimerStart
				(type == CTRL__TIMER_PERIODIC) ? pdTRUE : pdFALSE,
				(void *) arg,
				(TimerCallbackFunction_t)timeout_handler);
...
int serial_drv_close(struct serial_drv_handle_t** serial_drv_handle)
{
	if (!serial_drv_handle || !(*serial_drv_handle)) {
		...
		if (serial_drv_handle)
			esp_mem_free(serial_drv_handle);
		return ESP_FAIL;
	}
	esp_mem_free(*serial_drv_handle);
	return ESP_OK;
