...
static uint8_t * stream_get_00(uint8_t * stream, uint8_t offset, uint16_t len)
{
	//printf("----lyrdbg----%s:%u\n", __FILE__, __LINE__);
	(void)len;
	//printf("----lyrdbg----%s:%u\n", __FILE__, __LINE__);
...
static uint32_t arping_get_ipaddr(uint8_t *stream, ie_type_e type)
{
	...
	if (type == IP_ADDR_TYPE_SRC ) {
		...
		printf("----arping_get_ipaddr----%s:%u\n", __FILE__, __LINE__);
		// ip_nw = *(uint32_t*) stream_get(stream,
		// 		ARPING_OFFSET_DST_IP, IP_ADDR_LEN);
		ip_nw = *(uint32_t*) (stream + ARPING_OFFSET_DST_IP);
	} else {
		printf("Address type be either src/dst\n");
	}
...
print_hex_dump(pkt, pkt_len, "arp_req_handler pkt:");
